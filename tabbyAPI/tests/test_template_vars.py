...
def container(default=None, force=None, reasoning=None):
...
class ResolveTemplateVarsTests(unittest.TestCase):
    ...
    def test_reasoning_false_defaults_enable_thinking_off(self):
        out = resolve_template_vars(request(), container(reasoning=False))
        self.assertIs(out["enable_thinking"], False)

    def test_reasoning_false_still_allows_client_enable_thinking(self):
        out = resolve_template_vars(request(enable_thinking=True), container(reasoning=False))
        self.assertIs(out["enable_thinking"], True)

    def test_reasoning_true_does_not_force_enable_thinking(self):
        out = resolve_template_vars(request(), container(reasoning=True))
        self.assertNotIn("enable_thinking", out)
