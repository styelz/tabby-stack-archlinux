...
class ChatJsStopQueueSteerTests(unittest.TestCase):
    ...
    def test_code_mode_workspaces_nest_chats(self):
        ...
        self.assertIn("function workspacePinTarget(", self.src)
        self.assertIn('kind === "root"', self.src.split("function navRowTools")[1].split("function navRowHtml")[0])
        self.assertIn("const pin = kind === \"root\"", self.src.split("function navRowTools")[1].split("function navRowHtml")[0])
        self.assertIn("kind === \"root\" && item.pinned", self.src)
        self.assertIn("pinBtn.hidden = !root", self.src)
