...
| `mkfs.btrfs: Device or resource busy` on the root partition | A previous tsos run in this live session left the kernel holding btrfs after umount (lsblk looks clean). Current tsos (1.0.44) unloads that. Copy the new `tsos-installer.sh` onto the ISO and run it again, or reboot the live USB once then install. Do not use `--resume-tabby` here — that is only after Arch is already mounted at `/mnt`. |
